...
export const createOrder = asyncErrorHandler(
  async (req: CustomRequest, res) => {
    ...
    await doTransition(async (client) => {
      ...
      const shippingCharge = SHIPPING_CHARGE_STATIC;

      // if (priceAfterDiscount !== 0) {
      //   totalFinalAmount = priceAfterDiscount + (priceAfterDiscount * (GST_PERCENTAGE /100)) + shippingCharge;
      // } else {
      //   totalFinalAmount = subTotal + (subTotal * (GST_PERCENTAGE /100)) + shippingCharge;
      // }

      if(priceAfterDiscount !== 0) {
        totalFinalAmount = priceAfterDiscount + shippingCharge
      } else {
        totalFinalAmount += shippingCharge;
      }

      // now apply gst to final amount logic (productPrice - discount + shipping) + gst%
      totalFinalAmount = totalFinalAmount + (totalFinalAmount * (GST_PERCENTAGE / 100));
       ...
       SELECT
        ...
        LEFT JOIN LATERAL (
          SELECT
            name
          FROM addresses
          WHERE address_id = o.shipping_address_id
