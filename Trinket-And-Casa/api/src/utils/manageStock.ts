  ...
  if (
    ...
    if (order_status == ORDER_CONFIRMED) {
      const { rows, rowCount } = await pgClient.query(
        `
          SELECT
           ...
           COALESCE(JSON_AGG(a) FILTER (WHERE a.address_id IS NOT NULL), '[]'::json) AS shipping_details
          FROM orders o
      ...
      if (rowCount != 0) {
        ...
        const value = rows[0];
        if (value.shipping_details.length == 0) {
          throw new ErrorHandler(500, "Unable to fetch user address details");
        }
        ...
        sendEmail(rows[0].email, "ORDER_CONFIRMED_EMAIL", {
          ...
          items: rows[0].items,
          orderLink: `${process.env.FRONTEND_HOST_URL}/profile?tab=orders`,
        });
      }
    }
  } else if (
