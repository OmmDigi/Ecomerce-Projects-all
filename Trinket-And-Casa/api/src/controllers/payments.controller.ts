...
import { VUpdatePaymentStatus } from "../validator/payment.validator";
...
export const verifyPayment = asyncErrorHandler(async (req, res) => {
  ...
  if (gatewayName == "razorpay") {
    if(!req.body.razorpay_order_id) throw new ErrorHandler(400, "Razorpay Order Id Is Required!");

    const { items, count } = await fetchAnPaymentInfo(req.body.razorpay_order_id);
...
export const updatePaymentStatus = asyncErrorHandler(async (req, res) => {
  ...
  await doTransition(async (client) => {
    const { rowCount } = await client.query(`UPDATE orders SET payment_status = $1 WHERE order_id = $2 AND payment_method != 'ONLINE'`, [
      value.status,
      value.orderid,
    ]);

    if(rowCount == 0) throw new ErrorHandler(400, "Unable to update payment status as it's online payment");
