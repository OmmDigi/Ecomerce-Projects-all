...
export const createNewCategory = asyncErrorHandler(async (req, res) => {
  ...
  try {
    await pool.query(
      "INSERT INTO categories (name, slug, image, alt_tag) VALUES ($1, $2, $3, $4)",
      [value.name, value.slug, value.image, value.alt_tag ?? null]
    );
...
export const updateCateogry = asyncErrorHandler(async (req, res) => {
  ...
  try {
    await pool.query(
      "UPDATE categories SET name = $1, slug = $2, image = $3, alt_tag = $4 WHERE id = $5",
      [value.name, value.slug, value.image, value.alt_tag ?? null, value.id]
    );
...
export const getSingleRecipientList = asyncErrorHandler(async (req : CustomRequest, res) => {
...
export const updateRecipient = asyncErrorHandler(async (req, res) => {
  ...
  await pool.query(
    ...
    [value.tag_name, value.image, value.alt_tag ?? null, value.status, value.id]
  );
...
export const getProductList = asyncErrorHandler(
  async (req: CustomRequest, res) => {
    ...
    if (req.query.tag) {
      if (filter === "") {
        filter = `WHERE p.tags ILIKE '%' || $${placeholdernum++} || '%'`;
      } else {
        filter += ` AND p.tags ILIKE '%' || $${placeholdernum++} || '%'`;
      }
    ...
    if (req.query.status && req.token_info?.role == "Admin") {
  ...
  await doTransition(async (client) => {
    const { rowCount, rows } = await client.query(
      `INSERT INTO products 
        (sku_id, name, description, category_id, price, compare_at_price, available_quantity, meta_title, meta_description, status, tags, slug, sub_category_id)
       VALUES
...
export const createNewSubCategory = asyncErrorHandler(async (req, res) => {
  ...
  try {
    await pool.query(
      "INSERT INTO sub_categories (category_id, name, slug, image, alt_tag) VALUES ($1, $2, $3, $4, $5)",
      [
        value.category_id,
        value.name,
        value.slug,
...
export const updateSubCateogry = asyncErrorHandler(async (req, res) => {
  ...
  try {
    await pool.query(
      "UPDATE sub_categories SET name = $1, slug = $2, image = $3, alt_tag = $4, category_id = $5 WHERE id = $6",
      [
        value.name,
        value.slug,
        value.image,
...
export const createNewReview = asyncErrorHandler(
  async (req: CustomRequest, res) => {
    ...
    await pool.query(
      "INSERT INTO reviews (user_id, stars, message, product_id) VALUES ($1, $2, $3, $4)",
